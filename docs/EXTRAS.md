# Optional: Demo Video & Presentation

Demo videos and slide decks are **mostly for presentation purposes**. As a technical judge, the code and documentation are the primary evaluation material. For technical evaluation, refer to:

- [Project Overview](PROJECT.md) -- Problem, solution, business model, roadmap
- [Technical Documentation](TECHNICAL.md) -- Architecture, setup, demo guide

---

## Demo Video

> Link: _(to be added)_

A 3-5 minute walkthrough showing:
1. Starting the agent with `docker-compose up`
2. Registering via Telegram bot
3. Scanning markets for best yields
4. Depositing into Venus via natural language
5. Checking portfolio and trade history
6. Auto-arbitrage execution

---

## Slide Deck

> Link: _(to be added)_

---

## Live Demo Access

- **Telegram Bot**: [@defai_bharat_bot](https://t.me/defai_bharat_bot)
- **Dashboard**: _(deployment URL to be added)_
- **API Health**: _(deployment URL)_/api/health
- **Network**: BSC Testnet (Chain ID 97)

---

## Screenshots

_(to be added)_

- Dashboard overview with portfolio chart
- Telegram bot conversation showing yield deposit
- MCP tool execution in Claude Desktop
- Market scanner results
